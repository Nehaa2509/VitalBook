...
# ── Razorpay client (initialized from .env via settings) ──────────────────────
# Keys are blank by default; integration activates when keys are set in .env
_rzp_key = django_settings.RAZORPAY_KEY_ID
_rzp_secret = django_settings.RAZORPAY_KEY_SECRET
razorpay_client = razorpay.Client(auth=(_rzp_key, _rzp_secret)) if (_rzp_key and _rzp_secret) else None
...
def cancel_appointment(request, appointment_id):
    """
    Legacy cancel endpoint — redirects to the proper Razorpay-backed
    cancellation flow (request_cancellation) so fees are actually collected.
    """
    return redirect('request_cancellation', appointment_id=appointment_id)
...
def request_cancellation(request, appointment_id):
    ...
    - ₹200 Razorpay penalty if < 24 hours before appointment.
    """
    appointment = get_object_or_404(Appointment, id=appointment_id)
    ...
    # PENALTY Cancellation: less than 24 hours away
    cancellation_fee_paise = 200 * 100  # ₹200 in paise
    razorpay_order_id = ''
    ...
    context = {
        ...
        'razorpay_key': django_settings.RAZORPAY_KEY_ID or '',
        'razorpay_order_id': razorpay_order_id,
        'amount_paise': cancellation_fee_paise,
...
def verify_cancellation_payment(request, appointment_id):
    """
    Handles the Razorpay POST callback after cancellation fee payment.
    Verifies signature, cancels the appointment, and redirects to dashboard.
    """
    ...
    if request.method == 'POST':
        ...
        email_utils.send_appointment_cancelled(appointment, cancelled_by='patient')
        messages.success(request, 'Cancellation fee paid. Appointment has been cancelled.')
        return redirect('patient_dashboard')
...
def checkout(request, appointment_id):
    """Display Razorpay checkout page — creates a real Razorpay order."""
    appointment = get_object_or_404(Appointment, id=appointment_id)
    ...
    # Fallback to allow POST methods through gracefully if triggered by aggressive gateway hooks
    if request.method == 'POST':
        # Safely capture any POST attempts on checkout and bump directly to receipt or success logic
    ...
    # Create a Razorpay order if the client is configured
    razorpay_order_id = ''
    razorpay_configured = bool(razorpay_client)
    ...
    if razorpay_configured:
        try:
            ...
            messages.error(request, f'Could not connect to payment gateway: {e}. Please try again.')
            return redirect('my_appointments')
    ...
    context = {
        ...
        'amount_display':      doctor.consultation_fee,
        'razorpay_configured': razorpay_configured,
    }
...
def razorpay_payment_success(request):
    """Handles the POST callback from Razorpay after a successful payment.
    Verifies the cryptographic signature, creates Payment record, confirms appointment.
    """
    if request.method == 'GET':
        # Accept GET requests gracefully instead of throwing method errors
        return redirect('my_appointments')
    ...
    if request.method == 'POST':
        ...
        if not all([razorpay_order_id, razorpay_payment_id, razorpay_signature, appointment_id]):
            messages.error(request, 'Incomplete payment data received.')
        ...
        try:
            if razorpay_client:
                razorpay_client.utility.verify_payment_signature(params_dict)
            # If client not configured, we skip verification (dev mode)
        except razorpay.errors.SignatureVerificationError:
            messages.error(request, 'Payment verification failed! Signature mismatch.')
            return redirect('payment_failed')
        except Exception as e:
        ...
        # Idempotency: skip if already recorded (e.g. user refreshed)
        if appointment.payments.filter(razorpay_payment_id=razorpay_payment_id).exists():
            return redirect('payment_receipt', appointment_id=appointment.id)
        ...
        messages.success(request, '✅ Payment verified! Your appointment is confirmed.')
        return redirect('payment_receipt', appointment_id=appointment.id)
...
def payment_receipt(request, appointment_id):
    """Show a clean payment receipt after successful Razorpay transaction."""
    appointment = get_object_or_404(Appointment, id=appointment_id)
...
def payment_failed(request):
    """Shown when Razorpay signature verification fails."""
...
def confirm_appointment_admin(request, appointment_id):
    """Confirm an appointment from the changelist action button."""
    appointment = get_object_or_404(Appointment, id=appointment_id)
    ...
    try:
        ...
        pass
    messages.success(request, f'✅ Appointment #{appointment.id} confirmed. Patient notified.')
